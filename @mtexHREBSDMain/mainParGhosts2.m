function mainParGhosts2(obj, input, options)
    pool = gcp("nocreate");
    if isempty(pool)
    ...
    for i = start:numChunks
        ...
        for l = 1:options.ChunkSize
            ...
            if sum(strcmp({F.State}, 'finished'))% && length(F) > options.numCores
                if options.Verbose
                    clc
                    disp("Chunk # "+num2str(i));
                    disp(F)
...
%                 );
%         end
%         if length(F) >= 2*options.numCores
%             if options.Verbose
%                 clc
%                 disp("Chunk # "+num2str(i));
%                 disp(F)
...
function [i, out, k, range, errorMsg] = readFutures(F, k, chunk)
    ...
    try
        ...
        if strcmp(me.identifier, ...
            ...
            out = zeros(chunk,46);
            for m = 1:chunk
                out(1,m) = F(i).InputArguments{1,4}(m,1);
...
function out = Main(input, cfs, cfa, ids, skipStrain, skipDer)
%     if size(pats,1) > 1
%         pats =  patPackage(input, ids, skipe(l), skipd(l));
%         out = zeros(size(pats,1), 46);
%         for i = 1:size(pats,1)
%             out(i,:) = Main( ...
%                 input, cfs, cfa, pats(i,:), skipStrain(i), skipDer(i) ...
%                 );
%         end
%     else
        out = zeros(1,46);
...
%         out(1) = pats(1).scanIndex;
        pats =  patPackage(input, ids, skipStrain, skipDer);
        if ~skipStrain
            cfs.getBeta(pats(1), pats(2));
            out(2:10) = cfs.beta;
            out(11:19) = cfs.g;
            if cfs.AnalysisType == 2
                out(38:40) = cfs.patternCenter;
            else
