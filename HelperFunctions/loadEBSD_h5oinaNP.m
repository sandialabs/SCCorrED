function [ebsd] = loadEBSD_h5oinaNP(fname,varargin)
% This is a modified version of loadEBSD_h5oina that does not load
% processed patterns into memory. 2024-12-11
...
for k = 1 :length(EBSD_index) % TODO: find a good way to write out multiple datasets
    ...
    for thing = 1:length(EBSD_data.Datasets)
        ...
        dataSize = EBSD_data.Datasets(thing).Datatype.Size * ...
            prod(EBSD_data.Datasets(thing).Dataspace.Size);
        % Only read data less than 4 GB (2^32 bytes) in size
        % Do not read patterns
        if ~(strcmpi(sane_name,"Processed Patterns") || dataSize > 2^32)
            EBSDdata.(sane_name)=double(h5read(fname,[EBSD_data.Name '/' EBSD_data.Datasets(thing).Name]));
        else
            fprintf(strcat("Skipped """,EBSD_data.Name,'/', ...
                EBSD_data.Datasets(thing).Name,"""\n"));
        end
    end
    ...
    if size(namePairs,1) > 1 % Following loop breaks for single phase scans
