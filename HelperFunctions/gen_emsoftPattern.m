function image = gen_emsoftPattern(mtexHREBSD, pattern, Ftensor, applyDeformation)
...
%     end
    ...
    pc_p = pattern.patternCenter;
    EMdataPath = mtexHREBSD.paths.EMdataPath;
    EMsoftPath = mtexHREBSD.paths.EMsoftPath;
...
%     EMsoftPath = '\Users\wggilli\Documents\EMsoft-5.0.20230206.-Win64\bin\';
    ImageInd = pattern.scanIndex;
    if strcmp(pattern.material, 'Ni')
        Material = 'Nickel';
    else
        Material = pattern.material;
    end
    L = pc_p(3) * mtexHREBSD.scan.pixelSize * mtexHREBSD.scan.delta;
    thetac = mtexHREBSD.scan.cameraElevation;
    delta = mtexHREBSD.scan.delta;
    numsx = mtexHREBSD.scan.pixelSize;
    numsy = numsx;
    xpc = -(pc_p(1) - 0.5) * mtexHREBSD.scan.pixelSize;
    ypc = (pc_p(2) - 0.5) * mtexHREBSD.scan.pixelSize;
    omega = 0;
    alphaBD = 0;
    energymin = 5.0;
    energymax = 20.0;
...
%     Ftensor = '1.D0, 0.D0, 0.D0, 0.D0, 1.D0, 0.D0, 0.D0, 0.D0, 1.D0,';
    ...
    [phi1, PHI, phi2]=gmat2euler(q_emsoft'*pattern.g*q_emsoft);
%     [phi1, PHI, phi2]=gmat2euler(pattern.g);
