...
%             obj.C = mtexHREBSD.C.M*1E9;
        ...
        function beta = get_beta(obj, refPat, testPat, mtexHREBSD)
            rout = zeros(3, obj.numRois);
            qout = zeros(3, obj.numRois);
            for i = 1:obj.numRois
                ri_p = obj.rRef_p(:,i);
                shiftRoi_p = obj.get_shift(ri_p, obj.F_guess);
                shiftRoi = (obj.ft.phosphorFrame2imageVec(shiftRoi_p));
                [refRoi, rrange, crange] = mtexHREBSD.roi.get_squareRoi(...
                    refPat, i ...
                    );
                [testRoi, srrange, scrange] = ...
                    mtexHREBSD.roi.get_squareRoiShifted( ...
                        testPat, i, shiftRoi ...
                );
                ccMap = get_crossCorrelation(refRoi, testRoi, ...
                    mtexHREBSD.roi, 'Gradient', ...
                    mtexHREBSD.analysis.Gradient ...
                );
                ...
                qout(:,i) = qstar_p/norm(ri_p);
                rout(:,i) = ri_p/norm(ri_p);
            end
            [beta, ~, ~, ~, ~] = obj.calc_betaDetector( ...
                mtexHREBSD, rout, qout ...
            );
        end
        ...
        function [beta, fit, qFit, A4, b4] = calc_betaDetector(obj, mtexHREBSD, rs, qs)
            r1 = rs(1,:)';
            r2 = rs(2,:)';
            r3 = rs(3,:)';
            q1 = qs(1,:)';
