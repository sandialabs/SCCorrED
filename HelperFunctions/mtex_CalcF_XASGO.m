classdef mtexHREBSD_roiSettings
    ...
    methods
        function obj = mtexHREBSD_roiSettings(analysisSettings, pattern)
            ...
            obj.pixelSize = min(size(pattern.image));
        ...
        function roi = get_squareRoi(obj, pattern, roiInd)
            center = round(obj.centers(:,roiInd));
            h = round(obj.roiSize/2);
            rrange = (center(1)-h):(center(1)+h)-1;
            crange = (center(2)-h):(center(2)+h)-1;
            roi = pattern.image(rrange, crange);
        end


        function roi = get_squareRoiShifted(obj, pattern, roiInd, shift)
            center = round(obj.centers(:,roiInd) + shift);
            h = round(obj.roiSize/2);
            rrange = (center(1)-h):(center(1)+h)-1;
            crange = (center(2)-h):(center(2)+h)-1;
