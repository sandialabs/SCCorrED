function ccMap = get_normCrossCorrelation(ref_roi, test_roi, roiSettings, varargin)
...
%     args
    if args.Gradient
        ...
        ref_fft = ...
            roiSettings.custfilt.*fftn(roiSettings.windowfunc.*ref_roi);
        test_fft = ...
            roiSettings.custfilt.*fftn(roiSettings.windowfunc.*test_roi);
    end
    refMean = mean(ref_fft(:));
    refStd = std(ref_fft(:));
    testMean = mean(test_fft(:));
    testStd = std(test_fft(:));
    ccF = (ref_fft-refMean).*(test_fft - testMean)./(refStd.*testStd);
    ccMap = fftshift(ifftn(ccF));
%     ccMap = fftshift(ifftn(ref_fft.*conj(test_fft)));
%     ccMap = fftshift(ifftn(roiSettings.custfilt.*test_fft.*conj(roiSettings.custfilt.*ref_fft)));
    if args.Plot
        if args.Gradient
            plot_CC_gradient(ref_G, test_G, ccMap)
        else
            plot_CC(ref_roi, test_roi, real(ccMap))
        end
    end
end
